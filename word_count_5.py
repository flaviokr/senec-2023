...
def count_words(lines, chunk_start, chunk_end):
    words_dict = {}
...
def word_count(lines):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_procs) as executor:
        for chunk_start in range(0, num_lines, chunk_size):
            ...
            future = executor.submit(count_words, lines, chunk_start, chunk_end)
...
with open("./data/file.txt", "r") as file:
    for line in file:
        ...
        if len(lines_to_process) == 2_000_000:
            print(f"Reading {len(lines_to_process)} lines...")
            results.append(word_count(lines_to_process))
            lines_to_process = []
